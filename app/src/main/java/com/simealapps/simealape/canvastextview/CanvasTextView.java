...
import com.simealapps.simealape.canvastextview.CustomRelativeLayout.RemoveTextListener;
...
public class CanvasTextView extends View {
        ...
        if (td == null) {
            ...
            if (this.textData.getFontPath() != null) {
               // Typeface typeFace = FontCache.get(context, this.textData.getFontPath());
                Typeface typeFace =Typeface.createFromAsset(context.getAssets(),textData.getFontPath());
                if (typeFace != null) {
    ...
    public void setNewTextData(TextDataItem textData2) {
        this.textData = textData2;
        float exRight = this.rect.right;
        this.rect.right = this.rect.left + textData2.textPaint.measureText(textData2.message) + (2.0f * this.paddingWidth);
        this.scaleBitmapMatrix.postTranslate(this.rect.right - exRight, 0.0f);
        postInvalidate();
    }
    ...
    public boolean isOnCross(float x, float y) {
    ...
    public void createDeleteDialog(Context paramContext, final View paramView) {
        ...
        localBuilder.setMessage((int) R.string.collage_lib_delete_message).setCancelable(true).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            @Override
